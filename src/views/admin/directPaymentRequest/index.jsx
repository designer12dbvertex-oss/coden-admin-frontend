...
//     try {
//       await axios.post(
//         `${baseUrl}api/direct-order/admin/approve-release/${pendingUpdateData.orderId}/${pendingUpdateData.paymentId}`,
//         {
...
export default function OrdersTable() {
      ...
      try {
        ...
        const formattedData = response.data.data.map((item) => ({
          ...
          createdAt: item.createdAt
            ...
            : 'N/A', // Keeping current formatting for display
  ...
  const exportToCsv = () => {
    ...
    const headers = [
      ...
      'Platform Fee Paid',
      'Total Tax',
      // Add more headers for customer/service provider bank details if needed in the CSV
    ];
    ...
    allData.forEach((order, index) => {
      const row = [
        ...
        `"${order.paymentStatus}"`,
        `"${order.createdAt}"`, // Dates already formatted as strings
        `"${order.deadline}"`,   // Dates already formatted as strings
        order.platformFee,
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'release_requested_orders_data.csv');
