...
import {
  ...
  DeleteIcon,
  SearchIcon,
  DownloadIcon, // Added DownloadIcon for the export button
} from '@chakra-ui/icons';
import { toast, ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import defaultProfilePic from 'assets/img/profile/profile.webp';
...
console.log(userId);
	...
	if (data.status) {
	  ...
	  // You might want to re-fetch all users or update the specific user in the `data` state
	  // For simplicity, I'm just closing the modal. Re-fetching might be more robust.
...
useEffect(() => {
  if (selectedUser?.full_address) {
	...
	if (selectedUser.full_address.length > 0 && selectedUser.full_address[0].latitude && selectedUser.full_address[0].longitude) {
		setMapCenter({
			lat: selectedUser.full_address[0].latitude,
			lng: selectedUser.full_address[0].longitude,
		});
...
const addSelectedLocation = () => {
  ...
  // Function to export all filtered data to CSV
  const exportToCsv = () => {
	...
	const rows = filteredData.map((item, index) => [
	  ...
	  item.createdBy,
	  item.categoryName,
	  item.subcategoryNames.join('; '), // Join subcategories with a semicolon
	  item.location,
	  item.mobile,
	  item.referral_code,
	  item.createdAt,
	...
	link.setAttribute('download', 'both_list.csv');
	document.body.appendChild(link);
	link.click();
	document.body.removeChild(link);
	toast.success('Data exported to CSV!', { position: 'top-right' });
  };
	  ...
	  >
		...
		>
		  <Button
			colorScheme="teal"
			onClick={() => navigate('/admin/createServiceProvider')}
			mr={2} // Add some margin to separate from the new button
		  >
			Create Service Provider
		  </Button>
		  {/* Export to CSV Button */}
		  <Button
			colorScheme="blue" // You can choose a different color scheme
			onClick={exportToCsv}
