...
console.log(userId);
	...
	if (data.status) {
	  ...
	  closeUserModal();
	  // Assuming you have a way to re-fetch users after an update
	  // For now, let's just refresh the page or re-call fetchData
  ...
  useEffect(() => {
    ...
    const handleExportCsv = useCallback(() => {
        ...
        filteredData.forEach((row, index) => {
            const subcategoriesString = row.subcategoryNames.join('; ');
            const createdAtFormatted = row.createdAt ? new Date(row.createdAt).toISOString().split('T')[0] : 'N/A'; // Ensures YYYY-MM-DD
            const rowData = [
                index + 1,
                row.uniqueId,
                row.full_name,
                row.createdBy,
                row.categoryName,
                `"${subcategoriesString}"`, // Quote to handle commas in subcategory names
                `"${row.location}"`, // Quote to handle commas in address
                row.mobile,
                row.referral_code,
                createdAtFormatted,
                row.active ? 'Yes' : 'No',
		...
		>
		  ...
		  >
            ...
            <Button
                ...
                mt={{ base: '10px', md: '0' }} // Margin for responsiveness
            >
                Export 
            </Button>
		</Flex>
