...
export default function CollarManagement() {
  ...
  const fetchData = async () => {
    try {
      const [countryRes, collarRes] = await Promise.all([
        axios.get(`${baseUrl}api/country/getAll`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get(`${baseUrl}api/collar/getAll`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleCountryChange = async (countryId) => {
    ...
    try {
      const res = await axios.get(
        `${baseUrl}api/state/getByCountry/${countryId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleAddCollar = async () => {
    if (!collarName || !selectedCity) {
      return toast({
        title: 'Please select City and enter Collar name',
        status: 'warning',
      });
    }
    setLoading(true);
    try {
      ...
      toast({ title: 'Error adding collar', status: 'error' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card p="20px">
        ...
        >
          ...
          <InputGroup maxW="300px">
            ...
            <Input
              placeholder="Search collar or city..."
              value={searchTerm}
