...
import {
  ...
  Button, Input, FormControl, FormLabel, useToast, IconButton,
  Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton,
  useDisclosure, InputGroup, InputLeftElement, Select, Badge, Tooltip, Switch
} from '@chakra-ui/react';
import { MdEdit, MdDelete, MdSearch, MdInfoOutline, MdLibraryBooks } from 'react-icons/md';
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import Card from 'components/card/Card';
...
export default function ChapterManagement() {
  ...
  const handleCreate = async () => {
    if (!formData.subSubjectId || !formData.name) {
      return toast({ title: 'Sub-Subject aur Name zaroori hain', status: 'warning' });
    }
    setLoading(true);
    try {
      await axios.post(`${baseUrl}api/admin/chapters`, formData, { headers });
      toast({ title: 'Chapter Created!', status: 'success' });
      setFormData({ subSubjectId: '', name: '', description: '', weightage: 0, order: 0, isFreePreview: false });
      fetchData();
    } catch (err) {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`${baseUrl}api/admin/chapters/${editData._id}`, editData, { headers });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Kya aap is chapter ko permanently delete karna chahte hain?")) {
      try {
    ...
    <Box pt={{ base: '130px', md: '80px', xl: '80px' }}>
      ...
      <Card mb='20px' p='20px'>
        ...
        <Flex gap='15px' wrap="wrap">
          <FormControl width={{base: '100%', md: '30%'}}>
            <FormLabel fontSize="sm" fontWeight="700">Select Sub-Subject</FormLabel>
            <Select 
                placeholder='Select Sub-Subject' 
                value={formData.subSubjectId} 
                onChange={(e) => setFormData({...formData, subSubjectId: e.target.value})}
            >
          ...
          <FormControl width={{base: '100%', md: '15%'}}>
            <FormLabel fontSize="sm" fontWeight="700">Weightage (%)</FormLabel>
            <Input 
                type="number" 
          ...
          <Flex align="center" mt="30px">
            <Text fontSize="sm" fontWeight="700" mr="2">Free Preview?</Text>
            <Switch 
          ...
          <Button colorScheme='brand' mt="25px" onClick={handleCreate} isLoading={loading} width={{base: '100%', md: 'auto'}}>
            Save Chapter
          </Button>
        </Flex>
      </Card>
      ...
      <Card p='20px'>
        <Flex justify='space-between' align='center' mb='20px'>
          <Text color={textColor} fontSize='18px' fontWeight='700'>Chapters List</Text>
          <InputGroup maxW='300px'>
            <InputLeftElement pointerEvents='none'><MdSearch color='gray.300' /></InputLeftElement>
            <Input placeholder='Search chapter...' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </InputGroup>
        </Flex>
            ...
            <Tbody>
              {filteredData.map((item) => (
                <Tr key={item._id}>
                  ...
                  <Td>
                    <Badge colorScheme="purple" variant="subtle">{item.subSubjectId?.name || 'N/A'}</Badge>
                  </Td>
                  <Td>{item.weightage}%</Td>
                  <Td maxW="200px">
                    <Tooltip label={item.description}>
                      <Text isTruncated fontSize="sm">{item.description || '-'}</Text>
                    </Tooltip>
                  </Td>
                  <Td>
                  ...
                  <Td textAlign="right">
                    <IconButton variant="ghost" colorScheme='blue' icon={<MdEdit />} onClick={() => { setEditData(item); onOpen(); }} mr='2' />
            ...
            {editData && (
              <Flex direction="column" gap="4">
                <FormControl>
                  ...
                  <Select value={editData.subSubjectId?._id || editData.subSubjectId} onChange={(e) => setEditData({...editData, subSubjectId: e.target.value})}>
                    {subSubjects.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}
                  </Select>
                </FormControl>
                <FormControl>
                ...
                <Flex gap="4">
                    <FormControl>
                        ...
                        <Input type="number" value={editData.weightage} onChange={(e) => setEditData({...editData, weightage: e.target.value})} />
                    </FormControl>
                    <FormControl>
