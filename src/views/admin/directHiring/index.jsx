...
export default function OrdersTable() {
  const [sorting, setSorting] = React.useState([]);
  const [data, setData] = React.useState([]); // Stores all fetched data
  const [filteredData, setFilteredData] = React.useState([]); // Stores data after search filtering
  const [searchQuery, setSearchQuery] = React.useState('');
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState(null);
  const [selectedOrder, setSelectedOrder] = React.useState(null);
  ...
  const handleExportCsv = () => {
    ...
    link.setAttribute('download', 'direct_hiring_orders.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.success('Orders exported to CSV successfully!', { position: 'top-right', autoClose: 3000 });
  };
  ...
  const columns = [
    columnHelper.display({
      ...
      cell: ({ row }) => {
        const serialNumber = table.getState().pagination.pageIndex * table.getState().pagination.pageSize + row.index + 1;
        return (
          <Flex align="center">
            <Text color={textColor} fontSize="sm" fontWeight="700">
              {isNaN(serialNumber) ? 'N/A' : serialNumber}
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Flex gap="4" align="center"> {/* Added Flex for search and export button */}
            <InputGroup maxW={{ base: '100%', md: '300px' }}>
              <InputLeftElement pointerEvents="none">
                <Icon as={SearchIcon} color="gray.400" />
              </InputLeftElement>
            ...
            <Button colorScheme="green" size="md" onClick={handleExportCsv}> {/* Export Button */}
              Export
            </Button>
          </Flex>
        </Flex>
