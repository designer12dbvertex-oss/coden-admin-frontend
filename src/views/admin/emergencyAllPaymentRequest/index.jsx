...
//           `${baseUrl}api/admin/getAllRequestEmergencyOrders`,
//           {
  ...
  const exportToCsv = () => {
    ...
    allData.forEach((order, index) => {
      const row = [
        ...
        order.totalTax,
        `"${order.paymentStatus}"`,
        `"${order.createdAt}"`, // Dates are already strings here
        `"${order.deadline}"`,   // Dates are already strings here
        order.platformFee,
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'emergency_orders_data.csv');
