...
export default function OrdersTable() {
  const [sorting, setSorting] = React.useState([]);
  const [allData, setAllData] = React.useState([]); // Stores all fetched data
  const [data, setData] = React.useState([]); // Stores paginated and filtered data for the table
  const [searchQuery, setSearchQuery] = React.useState('');
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState(null);
  const [currentPage, setCurrentPage] = React.useState(1);
  ...
  const fetchCategories = async () => {
    try {
      ...
      setAllData(formattedData); // Store all data here
  ...
  React.useEffect(() => {
    if (allData.length > 0) {
      ...
      console.log('Current Page Data:', {
        ...
        setData([]); // Clear data if allData is empty
        setTotalPages(1);
    }
  }, [currentPage, allData, searchQuery]);
  ...
  const handleExportCsv = () => {
    const filteredDataForExport = allData.filter((category) =>
        category.name.toLowerCase().includes(searchQuery.toLowerCase())
    );

    if (filteredDataForExport.length === 0) {
        toast.info('No data to export.', { position: 'top-right', autoClose: 3000 });
        return;
    }

    const headers = ['S.No', 'Category Name', 'Subcategories Count'];
    const rows = filteredDataForExport.map((item, index) => [
        index + 1,
        item.name,
        subcategoryCounts[item.id] || 0,
    ]);

    const csvContent = [
        headers.join(','),
        ...rows.map(row => row.map(field => `"${field}"`).join(',')) // Quote fields to handle commas in names
    ].join('\n');

    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.setAttribute('download', 'work_categories.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.success('Categories exported to CSV successfully!', { position: 'top-right', autoClose: 3000 });
  };
  ...
  const columns = React.useMemo(
    ...
    [textColor, borderColor, subcategoryCounts, currentPage], // Added currentPage to re-render S.No correctly
  );
    ...
    >
      <Flex px="25px" mb="8px" justifyContent="space-between" align="center">
        ...
        <Flex gap="4" align="center">
          ...
          <Button colorScheme="green" size="sm" onClick={handleExportCsv}>
            Export 
          </Button>
          <Button colorScheme="teal" size="sm" onClick={onAddOpen}>
            Add New Category
