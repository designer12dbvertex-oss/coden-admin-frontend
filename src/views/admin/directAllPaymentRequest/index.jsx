      ...
      try {
        ...
        const formattedData = response.data.data.map((item) => ({
          ...
          paymentStatus: item.payment_status
            ...
            : 'Unknown',
          createdAt: item.createdAt
            ? new Date(item.createdAt).toLocaleDateString('en-CA') // Format as YYYY-MM-DD for CSV
            : 'N/A',
          address: item.address || 'N/A',
          title: item.title || 'N/A',
          description: item.description || 'N/A',
          deadline: item.deadline
            ? new Date(item.deadline).toLocaleDateString('en-CA') // Format as YYYY-MM-DD for CSV
            : 'N/A',
          paymentHistory: item.payment_history ? [item.payment_history] : [],
          customerBankDetails: item.user_id?.bankdetails || {},
          serviceProviderBankDetails:
...
>
  ...
  const exportToCsv = () => {
    ...
    const headers = [
      ...
      'Platform Fee',
      'Platform Fee Paid',
      // Add more headers as needed for customer/service provider bank details if required
    ];
    ...
    allData.forEach((order, index) => {
      const row = [
        index + 1,
        `"${order.orderId}"`, // Wrap in quotes to handle potential commas
        `"${order.orderType}"`,
        `"${order.title}"`,
        `"${order.customerName}"`,
        ...
        order.totalTax,
        `"${order.paymentStatus}"`,
        `"${order.createdAt}"`, // Dates already formatted as YYYY-MM-DD
        `"${order.deadline}"`,   // Dates already formatted as YYYY-MM-DD
        order.platformFee,
        `"${order.platformFeePaid}"`,
      ];
    ...
    <>
      ...
      >
        <Flex px="25px" mb="20px" justify="space-between" align="center">
          ...
          <Flex gap="4"> {/* Added a Flex container for search and export */}
            <FormControl w="200px">
              <Input
                placeholder="Search by Project ID or Customer"
                value={searchQuery}
