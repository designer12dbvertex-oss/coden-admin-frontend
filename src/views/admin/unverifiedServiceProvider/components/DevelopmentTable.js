...
console.log(userId);
    ...
    if (data.status) {
      ...
      closeUserModal();
      // You might want to re-fetch users or update the state here to reflect changes in the table
...
const addSelectedLocation = () => {
  ...
  const exportToCsv = () => {
    ...
    const headers = [
      "S.No", "Unique ID", "Full Name", "Created By", "Category", "Subcategories",
      "Saved Address", "Mobile", "Referral Code", "Created At", "Active", "Verified"
    ];

    const rows = data.map((item, index) => {
      const subcategoryNames = item.subcategoryNames.join(', ');
      return [
        index + 1,
        item.uniqueId,
        item.full_name,
        ...
        // Ensure numeric format for relevant fields, otherwise keep as string
        ...
        // For other string values, handle commas and quotes for CSV
        return `"${String(value).replace(/"/g, '""')}"`;
      }).join(',');
    });

    const csvContent = [
      headers.map(header => `"${header.replace(/"/g, '""')}"`).join(','), // Quote headers
      ...rows
    ].join('\n');
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'unverified_service_providers.csv');
      ...
      >
        ...
        >
          <Button
            colorScheme="teal"
            onClick={() => navigate('/admin/createServiceProvider')}
            mr={2} // Add some margin to the right of this button
          >
            Create Service Provider
          </Button>
