...
export default function OrdersTable() {
  const [sorting, setSorting] = React.useState([]);
  const [allData, setAllData] = React.useState([]); // This holds all categories, unfiltered
  const [data, setData] = React.useState([]); // This holds paginated and filtered data for the table
  const [searchQuery, setSearchQuery] = React.useState('');
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState(null);
  const [currentPage, setCurrentPage] = React.useState(1);
  ...
  const fetchCategories = async () => {
    try {
      ...
      const counts = {};
      for (const category of formattedData) { // Use formattedData (all categories) for subcategory counts
        const subcategories = await fetchSubcategories(category.id);
        counts[category.id] = subcategories.length;
      }
      setSubcategoryCounts(counts);
  ...
  }, [currentPage, allData, searchQuery]); // allData is now the source of truth for filtering
  ...
  const convertToCsv = (data) => {
    ...
    const headers = [
      'S.No', 'Category Name', 'Subcategory Count', 'Image URL' // Added Image URL for completeness
    ];
    ...
    const rows = data.map((item, index) => {
      ...
      const serialNo = (index + 1); // Serial number for the exported data
      const subcategoryCount = subcategoryCounts[item.id] || 0;

      return [
        escapeCsv(serialNo),
        escapeCsv(item.name),
        escapeCsv(subcategoryCount),
  ...
  const handleExportCsv = () => {
    // We want to export all filtered data, not just the current page
    const filteredData = allData.filter((category) =>
        category.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    link.setAttribute('download', 'work_categories_data.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    toast.success('Work categories data has been exported to CSV.', {
      position: 'top-right',
      autoClose: 3000,
    });
  };
