...
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { SearchIcon, DownloadIcon } from '@chakra-ui/icons'; // Import DownloadIcon and SearchIcon
...
// Optimistically update UI
  ...
  const exportToCsv = () => {
    const headers = columns
      ...
      .map((col) => {
        // Map column IDs to display names for headers
        switch (col.id) {
          case 'sno': return 'S.No';
          case 'orderId': return 'PROJECT ID';
          case 'title': return 'TITLE';
          case 'customerName': return 'CUSTOMER';
          case 'serviceProvider': return 'SERVICE PROVIDER';
          case 'remainingAmount': return 'USER PAID';
          case 'paymentStatus': return 'PAYMENT STATUS';
          default: return col.id; // Fallback for other columns
        }
      })
      .join(',');
    ...
    const csvRows = data.map((row, index) => {
      const values = columns
        ...
        .map((col) => {
          ...
          if (col.id === 'sno') {
            value = index + 1;
          } else if (col.id === 'remainingAmount') {
            value = `₹${row[col.id].toLocaleString()}`;
          } else {
            value = row[col.id];
          }

          // Handle potential commas and double quotes within data by quoting them
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'bidding_orders_payments.csv');
    ...
    <>
      ...
      >
        <Flex
          ...
          justify="space-between"
          align="center"
          direction={{ base: 'column', md: 'row' }} // Added for responsiveness
          gap={{ base: '10px', md: '0' }} // Added for responsiveness
        >
          <Text
            color={textColor}
            fontSize="22px"
          ...
          <Flex gap="4"> {/* Group search and export buttons */}
            <InputGroup maxW={{ base: '100%', md: '300px' }}> {/* Added InputGroup for search */}
              ...
              <Input
                ...
                bg={useColorModeValue('gray.100', 'gray.700')} // Added for consistency
                _focus={{
