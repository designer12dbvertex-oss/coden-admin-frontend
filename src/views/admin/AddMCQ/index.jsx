...
import {
  ...
  useDisclosure, InputGroup, InputLeftElement, Select, Badge, Tooltip, Switch, Textarea, SimpleGrid
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        marks: 4, negativeMarks: 1, previousYearTag: false
      });
      fetchData();
    } catch (err) {
    ...
    <Box pt={{ base: '130px', md: '80px', xl: '80px' }}>
      ...
      <Card mb='20px' p='20px'>
        ...
        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={5}>
          <Box>
            <FormControl mb="3">
              <FormLabel fontSize="sm" fontWeight="700">Select Chapter</FormLabel>
            ...
            <FormControl mb="3">
              <FormLabel fontSize="sm" fontWeight="700">Question Text</FormLabel>
              <Textarea value={formData.question} onChange={(e) => setFormData({...formData, question: e.target.value})} placeholder='Enter question here...' />
            </FormControl>

            <SimpleGrid columns={2} spacing={3} mb="3">
              <FormControl>
                <FormLabel fontSize="sm" fontWeight="700">Difficulty</FormLabel>
              ...
              <FormControl>
                <FormLabel fontSize="sm" fontWeight="700">PYQ Tag?</FormLabel>
                <Flex align="center" h="40px">
                    <Switch isChecked={formData.previousYearTag} onChange={(e) => setFormData({...formData, previousYearTag: e.target.checked})} />
                    <Text ml="2" fontSize="xs">Previous Year Question</Text>
                </Flex>
              </FormControl>
            </SimpleGrid>
          </Box>
          ...
          <Box>
            <FormLabel fontSize="sm" fontWeight="700">Options (Enter at least 2)</FormLabel>
            ...
            <FormControl mt="3">
              <FormLabel fontSize="sm" fontWeight="700" color="brand.500">Correct Answer Text</FormLabel>
              <Input value={formData.correctAnswer} onChange={(e) => setFormData({...formData, correctAnswer: e.target.value})} placeholder='Must match one of the options' />
            </FormControl>

            <FormControl mt="3">
              <FormLabel fontSize="sm" fontWeight="700">Explanation</FormLabel>
              <Input value={formData.explanation} onChange={(e) => setFormData({...formData, explanation: e.target.value})} placeholder='Why is this answer correct?' />
            </FormControl>
          </Box>
        </SimpleGrid>

        <Button colorScheme='brand' mt="5" onClick={handleCreate} isLoading={loading} w="full">
          Create MCQ
        </Button>
      </Card>
      ...
      <Card p='20px'>
        <Flex justify='space-between' align='center' mb='20px'>
          <Text color={textColor} fontSize='18px' fontWeight='700'>MCQs List ({filteredData.length})</Text>
          <InputGroup maxW='300px'>
            <InputLeftElement pointerEvents='none'><MdSearch color='gray.300' /></InputLeftElement>
            <Input placeholder='Search MCQs...' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </InputGroup>
        </Flex>
                ...
                <Tr key={item._id}>
                  <Td maxW="300px">
                    <Tooltip label={item.question}>
                      <Text isTruncated fontWeight="700" color={textColor}>{item.question}</Text>
                    </Tooltip>
                  </Td>
                  <Td><Badge variant="outline">{item.chapterId?.name || 'N/A'}</Badge></Td>
                  <Td>
                    <Badge colorScheme={item.difficulty === 'hard' ? 'red' : item.difficulty === 'medium' ? 'orange' : 'green'}>
                      {item.difficulty}
                    </Badge>
                  </Td>
                  <Td>{item.marks} / -{item.negativeMarks}</Td>
                  <Td>{item.previousYearTag ? <Badge colorScheme="purple">PYQ</Badge> : '-'}</Td>
                  <Td textAlign="right">
