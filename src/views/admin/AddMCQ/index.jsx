...
export default function MCQManagement() {
    ...
    const fetchAll = async () => {
      try {
        const [courseRes, tagRes, mcqRes, testsRes] = await Promise.all([
          ...
          axios.get(
            finalMode === 'manual'
              ? `${baseUrl}/api/admin/mcqs?testId=null`
              : `${baseUrl}/api/admin/mcqs?testMode=${finalMode}`,
            { headers },
          ),
  ...
  const handleTopicAutoFill = async (rawId) => {
    try {
      ...
      const subSubject = chapter?.subSubjectId;
            ...
            {finalMode === 'exam' && (
              <FormControl>
                <FormLabel fontSize="sm" fontWeight="700">
                  Enter Topic ID (Auto Fill)
                </FormLabel>

                <Input
                  placeholder="Paste Topic ID here..."
                  value={topicIdInput}
                  onChange={(e) => {
                    const value = e.target.value.trim();
                    setTopicIdInput(value);
