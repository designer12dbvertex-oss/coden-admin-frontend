...
//           paymentStatus: item.service_payment?.payment_status || 'Unknown',
...
import { ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { SearchIcon, DownloadIcon } from '@chakra-ui/icons'; // Import DownloadIcon
...
export default function OrdersTable() {
  ...
  const exportToCsv = () => {
    const headers = columns
      ...
      .map((col) => {
        // Use the header text for CSV headers
        if (typeof col.header === 'function') {
          // This is a bit tricky with columnHelper.display headers,
          // but for columnHelper.accessor it would be easier.
          // For now, we'll try to extract text from Text component if possible.
          // Fallback to ID if not easily extractable.
          if (col.id === 'sno') return 'S.No';
          if (col.id === 'orderId') return 'PROJECT ID';
          if (col.id === 'customerName') return 'CUSTOMER';
          if (col.id === 'serviceProvider') return 'SERVICE PROVIDER';
          if (col.id === 'paidAmount') return 'PAID AMOUNT';
          if (col.id === 'hireStatus') return 'HIRE STATUS';
          if (col.id === 'createdAt') return 'CREATED AT';
          if (col.id === 'pendingReleasePayments') return 'PENDING RELEASE PAYMENTS';
          return col.id;
        }
        return col.header;
      })
      ...
      const values = columns
        ...
        .map((col) => {
          ...
          if (col.id === 'sno') {
            ...
            value = `₹${row[col.id].toLocaleString()}`;
          } else if (col.id === 'pendingReleasePayments') {
            value = row.pendingReleasePayments;
          }
          else {
            value = row[col.id];
          }
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'emergency_orders.csv');
