      ...
      try {
        ...
        setData(
          response.data.users.map((user) => ({
            ...
            mobile: user.phone || 'N/A',
            createdAt: user.createdAt
              ? new Date(user.createdAt) // Store as Date object for formatting later
              : null,
            referral_code: user.referral_code || 'N/A',
            createdBy: user.createdBy?.full_name || 'Self-Registered',
            verified: user.verificationStatus ?? false,
...
console.log(userId);
    ...
    if (data.status) {
      ...
      fetchUsers(); // Assuming you have a way to re-fetch data
    } else {
      alert(data.message || "Something went wrong");
    }
  } catch (err) {
...
useEffect(() => {
  if (selectedUser?.full_address) {
    setUserAddresses(selectedUser.full_address);
    // Set map center to the first address if available, otherwise default
    if (selectedUser.full_address.length > 0) {
      setMapCenter({
        lat: selectedUser.full_address[0].latitude,
        lng: selectedUser.full_address[0].longitude,
      });
...
const addSelectedLocation = () => {
  ...
  const exportToCsv = useCallback(() => {
    ...
    const headers = [
      "S.No", "Unique ID", "Full Name", "Created By", "Category", 
      "Subcategories", "Saved Address", "Mobile", "Referral Code", 
      "Created At", "Active", "Verified"
    ];
    ...
    filteredData.forEach((row, index) => {
      ...
      const subcategoryNames = row.subcategoryNames?.join('; ') || '';
      const location = row.location || '';
      const mobile = row.mobile || '';
      const referralCode = row.referral_code || '';
      const createdAt = row.createdAt ? format(row.createdAt, 'yyyy-MM-dd') : ''; // Format date
      const active = row.active ? 'Yes' : 'No';
      const verified = row.verified || '';

      csvRows.push(
        [
          sno, uniqueId, fullName, createdBy, categoryName, subcategoryNames,
          location, mobile, referralCode, createdAt, active, verified
    ...
    link.setAttribute('download', 'service_providers.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.success("Data exported to CSV successfully!", { position: "top-right" });
  }, [filteredData]);
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('createdAt', {
        ...
        cell: (info) => (
          <Text color={textColor} fontSize="sm" fontWeight="700" whiteSpace="nowrap">
            {info.getValue() ? format(info.getValue(), 'yyyy-MM-dd') : 'N/A'}
          </Text>
        ),
      }),
      columnHelper.accessor('active', {
      ...
      >
        ...
        >
          <Button
            colorScheme="teal"
            onClick={() => navigate('/admin/createServiceProvider')}
            mr={3} // Add margin to separate from export button
          >
            Create Service Provider
          </Button>
          <Button
          ...
          <ModalBody>
            {selectedUser && (
              <VStack align="start" spacing={4}>
                ...
                <Text>
                  <strong>Created At:</strong>{' '}
                  {selectedUser.createdAt ? format(new Date(selectedUser.createdAt), 'yyyy-MM-dd') : 'N/A'}
                </Text>
                <Text>
                  <strong>Updated At:</strong>{' '}
                  {selectedUser.updatedAt ? format(new Date(selectedUser.updatedAt), 'yyyy-MM-dd') : 'N/A'}
                </Text>
                {selectedUser.hiswork && selectedUser.hiswork.length > 0 && (
                  <>
                    <Text fontWeight="bold">Work Samples:</Text>
