...
const useFetchCategories = (baseUrl, token, selectedCategoryId) => {
	...
	const [loading, setLoading] = useState({ categories: false, subcategories: false });
	const [error, setError] = useState(null);
...
export default function ComplexTable() {
	...
	const openCategoryModal = useCallback((userId, currentCategory, currentSubcategories) => {
		setSelectedUserId(userId);
			...
			setData((prevData) =>
				prevData.map((user) =>
					user.id === selectedUserId
						? {
								...user,
								categoryName: categories.find((cat) => cat._id === selectedCategory)?.name || 'N/A',
							...
							<Button
								...
								onClick={() =>
									openCategoryModal(
										...
										info.row.original.userDetails.subcategory_ids?.map(sub => sub._id) || []
									)
								}
