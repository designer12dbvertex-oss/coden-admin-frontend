...
const useFetchCategories = (baseUrl, token, selectedCategoryId) => {
  ...
  useEffect(() => {
    ...
    fetchSubcategories();
  }, [baseUrl, token, selectedCategoryId]);
...
export default function ComplexTable() {
  ...
  } = useFetchCategories(baseUrl, token, selectedCategory); // Pass selectedCategory
  ...
  const openCategoryModal = useCallback(
    (userId, currentCategory, currentSubcategories) => {
      setSelectedUserId(userId);
  ...
  const handleCategorySubmit = useCallback(async () => {
    ...
    setToggleLoading((prev) => ({ ...prev, [selectedUserId]: true }));
    const success = await updateUserCategory(
      baseUrl,
      token,
      selectedUserId,
      selectedCategory,
              ...
              <>
                ...
                <FormControl>
                  ...
                  {categoriesLoading.subcategories ? (
                    ...
                    <>
                      {/* Display selected subcategories in an Input field */}
                      {/* Dropdown for selecting subcategories */}
                      <Select
                        ...
                        onChange={(e) => {
                          ...
                          setSelectedSubcategories((prev) => {
                            const newSelection = selectedOptions[0]
                              ? [...new Set([...prev, selectedOptions[0]])] // Add new subcategory if not already selected
                              : prev;
                            return newSelection;
                          });
                      ...
                      {/* Display selected subcategories as removable tags */}
